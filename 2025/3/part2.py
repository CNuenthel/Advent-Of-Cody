...
def extract_max_joltage(battery: str) -> int:
    ...
    while True:
        ...
        if len(max_joltage) == 11:  # Handle large remaining slice to choose from for last
            max_joltage.append(max(ls_battery))
            break
        elif not selectable_slice:  # Remaining slice holds exactly required nums to create 12-digit code
